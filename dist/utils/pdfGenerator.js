...
function generateDocumentPDF(data, type) {
    return new Promise((resolve, reject) => {
        try {
            const dirPath = path_1.default.join(process.cwd(), "public", "pdfs");
            ...
            doc
                .fillColor("#9ca3af")
                .fontSize(8)
                .text("Thank you for doing business with VendorBridge. This is a computer-generated document.", 50, 700, { align: "center", width: 500 });
            doc.end();
            stream.on("finish", () => {
                logger_1.logger.info(`PDF successfully written: ${filePath}`);
            ...
            stream.on("error", (err) => {
                logger_1.logger.error(`Error writing PDF file stream: %O`, err);
                reject(err);
            });
        }
        catch (error) {
            logger_1.logger.error(`Failed to generate PDF: %O`, error);
            reject(error);
        }
    });
}
